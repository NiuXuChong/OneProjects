...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    //外界需要使用DDMenu
    self.ddMenuController = rootController;
    //4.创建MenuController（我们自己创建的）
...
//    如果将来要分享到QQ或者微信
//    [UMSocialQQHandler setQQWithAppId:@"" appKey:@"" url:@"分享的链接"];
    return YES;
}

//接收到远程操控事件（只要在锁屏下点了播放或者暂停等 都会触发）
- (void)removeControlReceivedWithEvent:(UIEvent *)event{
  //  NSLog(@"%@", event);
    //将事件传回控制器中.对比看是触发什么事件
    self.Block(event);
}
...
- (NSURL *)applicationDocumentsDirectory {
    // The directory the application uses to store the Core Data store file. This code uses a directory named "COM.LANOU.WWW.OneProject" in the application's documents directory.
